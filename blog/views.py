...
import datetime # datetime neyar jonno
...
def index(request):
	...
	week_ago = datetime.date.today() - datetime.timedelta(days = 7) # sat din ber korar jonno
	trends = Post.objects.filter(time_upload__gte = week_ago).order_by('-read') # all post theke sat dine besi read post niye
...
def about(request):
	...
	diction = {'title': 'About ',}
	return render(request, 'about.html', diction)
